...
def fig_jitter():
    ...
    ax[0, 0].set_xlabel(r"disorder $c$ (fraction of $\Delta$)"); ax[0, 0].set_ylabel(r"Poisson Error")
    ...
    ax[0, 1].set_xlabel(r"disorder $c$"); ax[0, 1].set_ylabel(r"denominator / trace error ratio")
    ax[0, 1].set_title(r"(b) denominator edge widens with anisotropy")

    ax[1, 0].plot(jits, cond, "o-", color="tab:orange")
    ax[1, 0].set_xlabel(r"disorder $c$"); ax[1, 0].set_ylabel(r"$\max\,\mathrm{cond}(\mathbf{M})$")
    ...
    ax[1, 1].set_xlabel(r"disorder $c$"); ax[1, 1].set_ylabel(r"activation")
...
def fig_kernel_radius():
    ...
    ax[2].set_title(r"(c) min neighbour count (disorder $60\%$)"); ax[2].legend()
    fig.suptitle(r"Kernel shape \& support radius robustness (disorder $60\%$)")
    fig.tight_layout()
...
def fig_error_field():
    ...
    sc = ax.scatter(r["pos"][:, 0], r["pos"][:, 1], c=err, s=11, cmap="viridis")
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(OUT, "fig_error_field.png"), dpi=150)
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    fig_error_field(); print("saved fig_error_field.png")
